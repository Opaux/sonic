/*
 * CODE NOT CURRENTLY WORKING IN THE BOX, ~300Hz RESONATIONS IN THE BOX OCCURRING
 ...
 *
 * ANC test uses broadband LFSR noise.
 */
...
static inline int32_t noise_next_s24(void) {
    ...
    float white = (float)(int32_t)noise_lfsr / 2147483648.0f;
    return float_to_s24(white * NOISE_AMPLITUDE);
}
...
static void fxlms_init(void) {
    ...
    xil_printf("ANC test noise: broadband LFSR\r\n");
}
...
int main(void) {
    xil_printf("\r\n=== FXLMS ANC (Serial Telemetry — Broadband LFSR Noise Test) ===\r\n");
	...
	while (1) {
		...
		if (!anc.anc_active) {
		    ...
		    if (anc.train_blocks >= SPM_TRAIN_BLOCKS) {
		        anc.anc_active = 1;
		        xil_printf("SPM training complete — ANC active\r\n");
		    }
		}
    }
