 ...
 *
 * Modified: ANC test uses a sine wave instead of broadband LFSR noise.
 */
...
#define SPM_TRAIN_BLOCKS  128000
#define SPM_ENERGY_THRESH 1e-6f

// Sine-wave test tone parameters
#define SINE_FREQ_HZ    500.0f
#define SINE_AMPLITUDE  0.15f        // peak amplitude in [-1, 1] range
...
static const float sine_phase_inc = (2.0f * 3.14159265358979323846f * SINE_FREQ_HZ) / (float)FS;
...
// Sine-wave generator: returns an s24 sample and advances the phase
static inline int32_t sine_next_s24(void) {
    float sample = SINE_AMPLITUDE * sinf(sine_phase);
    sine_phase += sine_phase_inc;
    if (sine_phase >= 2.0f * 3.14159265358979323846f)
        sine_phase -= 2.0f * 3.14159265358979323846f;
    return float_to_s24(sample) >> 2;
}
...
static void fxlms_init(void) {
    ...
    xil_printf("ANC test tone: %d Hz sine wave\r\n", (int)SINE_FREQ_HZ);
}
...
static void process_block(int buf_idx) {
    ...
    for (int i = 0; i < BLOCK_SIZE; i++) {
    	int32_t x_raw;
			if (anc.anc_active)
				x_raw = sine_next_s24();   // <-- sine wave instead of LFSR
			else {
				noise_next_s24();   // keep LFSR advancing so sequence is deterministic
				x_raw = 0;
			}
...
int main(void) {
    xil_printf("\r\n=== FXLMS ANC (Serial Telemetry Version — Sine Wave Test) ===\r\n");
